 ...
 *
 * <p>
 * Note that all configurations are not effective at runtime unless and otherwise
 * specified.
 * </p>
 *
 */
...
public interface EurekaClientConfig {
    ...
    /**
     * Indicates how oftern(in seconds) to fetch the registry information from
     * eureka server.
     * @return
     */
     ...
     *
     * <p>
     * Eureka server could be added or removed and this setting controls how
     ...
     *
     * <p>
     * This may be needed for application which needs additional resiliency for
     ...
     * contract eureka server when the list of eureka servers come from the DNS.
     * This information is not required if the contract returns the service urls
     * by implementing (@link #getEurekaServerUrls(String))
     * <p>
     ...
     * </p>
     * @return  the string indicating the context {@link java.net.URL} of the eureka server.
     */
     ...
     * This information is not required if the contract returns the service urls
     * by implementing {@link #getEurekaServerServiceUrls(String)}
     * @return
     */
    ...
    /**
     * Indicate whether the client should explicitly unregister itself from the remote
    ...
    /**
     * Indicate whether or not this instance should try to use the eureka
     * server in the same zone for latency an/or other reason.
     *
     * <p>Ideally eureka client are configured to talk to servers in the same zone</p>
     *
     * <p>
     * <em>The changes are effective at runtime at the next registry fetch cycle as specified
    ...
    /**
     * Indicates whether server can redirect client request to a backup server/cluster.
     ...
     * out same information. Reconciliation failure could happen when the
     ...
     * the difference between the client and the server and this setting controls that.
     * </p>
     *
     * <p>
     ...
     * </p>
     *
     * @return true if the eureka client should be log delta differences in the
     ...
     * <p>
     * Note that the delta fetched can reduce the traffic tremendously, because
     ...
     * </p>
     *
     * @return true to enable fetching delta information for registry, false to get the
     * full registry.
     */
     ...
     * It is mandatory to define the availability zones for each of these regions as returned
     * by {@link #getAvailabilityZones(String)}. Failing to do so, will result in failure of\
     ...
     * <p>
     * <em>The changes are effective at runtime at the next registry fetch cycle as specified
     * by {@link #getRegistryFetchIntervalSeconds()}</em>
     * </p>
     ...
     *
     * <p>
     * Typically the eureka server {@link java.net.URL}s carry protocol,host,port,context and
     ...
     * @return the list of eureka server service urls for eureka client to talk to
     */
     ...
     * since the firewall cleans up the connection information after a few mins leaving the
     * connection hanging in limbo.
     * </p>
    ...
    /**
     * If set to true, the {@link EurekaClient} initialization should throw an exception at
     * constructor time if the initial fetch of eureka registry information from the remote
     * servers is unsuccessful.
     *
     * Note that if {@link #shouldFetchRegistry()} is set to false, then this config a no-op
    ...
    /**
     * Indicate whether the client is only interested in the registry information for a single VIP
     * @return the address of the VIP(name:port).
     * null if single VIP interest is not present.
     */
    @Nullable
    ...
    /**
     * Get a replacement string for Dollar sign <code>$</code> during serializing/deserializing
     * information in eureka
     * @return replacement string for Dollar sign <code>$</code>
     */
    String getDollarReplacement();

    /**
     * Get a replacement string for underscore sign <code>_</code> during serializing/deserializing
     * information in eureka.
    ...
    /**
     * If set to true, the {@link EurekaClient} initialization should throw an exception at
     * constructor time if an initial registration to the remote servers is unsuccessful.
     *
     ...
     *
     * @return the class name of the encoding codec to use for the client. If none set a
     * default codec will be used
     */
    String getEncoderName();

    /**
     * This is a transient config and once the latest codec are stable, can be removed
     * (as there will only be one)
     *
     * @return the class name of the decoding codec to use for the client. If none set a
     * default codec will be used
     */
    ...
    /**
     * To avoid configuration API pollution when trying new/experimental or features or
     * for the migration process, the corresponding configuration can be put into experimental
     * configuration section. Config format is:eureka.experimental.freeFormConfigString
     * @param name
     * @return a peoperty of experimental feature
     */
